...
module = model#DDP(model, device_ids=[local_rank], output_device=local_rank)
...
for i in range(2):
    ...
    for e in event_list:
        if e.cpu_parent is None and e.cuda_time_total != 0:
            key = e.name + str(count)
            if key not in results:
                results[key] = []
            results[key].append(e.cuda_time_total)
            results[key].append(str(e))
            self_cuda_time_total += e.cuda_time_total
...
#     print(result)
json.dump(results, open('event_single' + str(FLAGS.local_rank) + '.json', 'w'), indent=4)
